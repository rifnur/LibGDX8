...
public class GameMap {
    ...
    public GameMap() {
        ...
        this.fruitTexture = Assets.getInstance().getAtlas().findRegion("fruit");
...
//                if (data[i][j].dropType == DropType.FRUIT) {
//                    batch.draw(fruitTexture, i * CELL_SIZE, j * CELL_SIZE);
//                }
            }
        }
    ...
    public void generateFruit(int cellX, int cellY, int power) {
        if (MathUtils.random() < 1f) {
            DropType randomDropType = DropType.FRUIT;

            if (randomDropType == DropType.FRUIT) {
                int fruitAmount = power + MathUtils.random(power, power * 3);
                data[cellX][cellY].dropType = randomDropType;
                data[cellX][cellY].dropFruit = fruitAmount;
            }
        }
    }
    ...
    public boolean hasDropInCellFruit(int cellX, int cellY) {
        return data[cellX][cellY].dropType == DropType.FRUIT;
    }

    public void checkAndTakeDrop(Unit unit) {
        Cell currentCell = data[unit.getCellX()][unit.getCellY()];
        if (currentCell.dropType == DropType.NONE && currentCell.dropType == DropType.FRUIT) {
            return;
        }
    ...
    //съедаем фрукт
    //не ест
    public void checkAndTakeDropFruit(Unit unit) {
        Cell currentCell =data[unit.getCellX()][unit.getCellY()];
        if ( currentCell.dropType != DropType.NONE && hasDropInCellFruit(gc.getCursorX(),gc.getCursorY())) {


        if (currentCell.dropType == DropType.NONE) {
            return;
        }
        if (currentCell.dropType == DropType.FRUIT) {
            unit.addGold(currentCell.dropFruit);
        }
        currentCell.dropType = DropType.NONE;
        currentCell.dropFruit = 0;
    }
    }
}
